...
- **Filling the Sessions and Patterns pages with past demo history:** seed a
  ...
  generated shape without touching the DB. By default the web history/insights
  pages show all completed sessions, including this seeded history and any
  tracker sessions you end through the live agent. Set
  `FLOW_HISTORY_DEVICE_ID=demo-device` for a seed-history-only demo view.
